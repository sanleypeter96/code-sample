...
public class Submit {
	...
	private String[] getChallenge(String email, int partId) {
		...
		try {
			...
			out.write("&assignment_part_sid="
					+ String.format("%s-%s", homework_id(), partId));
	...
	private String submitSolution(String email, String ch_resp, int part,
		...
		try {
			...
			post.append("assignment_part_sid="
					+ URLEncoder.encode(
							String.format("%s-%d", homework_id(), part),
							"UTF-8"));
	...
	public String passXML() {
		...
		try {
			...
			System.err.println("Error submitting part 1 from filename '"
					+ fname + "'. Error: " + e.getMessage());
			return null;
		}
	}
	...
	public String gradeWiki(int partId, String ch_aux) {
		...
		if (partId == 1) {
			...
			wives = new ArrayList<String>(Arrays.asList(Arrays.copyOfRange(
					wivesSplit, 1, wivesSplit.length)));
		}
		String wikiFile = "../data/small-wiki.xml";
		List<String> infoBoxHusbands = wiki.processFile(new File(wikiFile),
				wives, true);
		File mod_file = modifyWiki(ch_aux, wikiFile);
		List<String> noInfoHusbands = wiki.processFile(new File(wikiFile), wives, true);
		mod_file.delete();
		System.setOut(out);
		System.setErr(err);
		if (partId == 1) {
	...
	public String packageWiki(List<String> infoBoxHusbands,
			List<String> noInfoHusbands) {
	...
	public String encodeWiki(List<String> infoBoxHusbands,
			List<String> noInfoHusbands) {
	...
	int scoreAnswers(List<Googling.Location> guesses,
			List<Googling.LocationPossibilities> gold, List<String> landmarks) {
