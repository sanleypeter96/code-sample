...
public class HolbrookCorpus {
	...
	List<Sentence> readHolbrook(String fileName) {
		...
		try {
			...
			System.err
					.println(String
							.format("[HolbrookReader.readHolbrook] Error reading corpus %s: %s",
	...
	Sentence processLine(String line) {
		...
		line = line.toLowerCase().trim().replace("\"", "").replace(",", "")
		...
		while (i < tokens.length) {
			...
			if (token.equals("<err")) {
				...
				String[] errorTokens = Arrays.copyOfRange(tokens, i,
						errorEnd + 1);
	...
	// Finds the next occurence of query in tokens, starting from index start.
	// -1 if missing.
	...
	// prints out corpus, split by lines, no <s> or punctuation, spaces between
	// each token.
	...
	// generates test sentences, then gets clean sentence for each one, JSONs
	// it.
